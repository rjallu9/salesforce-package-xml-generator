            ...
            switch (message.command) {
                ...
                case 'deploy':
                    ...
                    validateSession(message.destOrgId)
                        .then((result) => {
                        if (result.valid) {
                            retrieve(message.sourceOrgId, message.packagexml).then((result) => {
                                ...
                                let intervalId = setInterval(() => {
                                    retrieveStatus(message.sourceOrgId, retrieveJobId).then((result) => {
                                        if (result.done === 'true') {
                                            ...
                                            if (!isCancelDeploy) {
                                                ...
                                                deploy(message.destOrgId, result.zipFile, message.checkOnly, message.testLevel, message.testClasses).then((result) => {
                                                    ...
                                                    let deployIntervalId = setInterval(() => {
                                                        ...
                                                        deployStatus(message.destOrgId, deployJobId).then((result) => {
                                                            if (result.done === 'true') {
                ...
                case 'quickDeploy':
                    ...
                    quickDeploy(message.destOrgId, message.id).then((result) => {
                        let deployJobId = result;
                        let intervalId = setInterval(() => {
                            deployStatus(message.destOrgId, deployJobId).then((result) => {
                                if (result.done === 'true') {
                ...
                case 'compare':
                    ...
                    validateSession(message.destOrgId)
                        .then((result) => {
                        if (result.valid) {
                            retrieve(message.destOrgId, message.packagexml).then((result) => {
                                let destRetrieveJobId = result;
                                let destIntervalId = setInterval(() => {
                                    retrieveStatus(message.destOrgId, destRetrieveJobId).then((result) => {
                                        if (result.done === 'true') {
                ...
                case 'delete':
                    ...
                    deleteMD(message.sourceOrgId, message.packagexml).then((result) => {
                        panel.webview.postMessage({ command: 'deployStatus', result: { stage: "deployment", message: 'Deployment Initiated' } });
                        let deployJobId = result;
                        let deployIntervalId = setInterval(() => {
                            if (isCancelDeploy) {
                            ...
                            deployStatus(message.sourceOrgId, deployJobId).then((result) => {
                                if (result.done === 'true') {
                                    clearInterval(deployIntervalId);
                                }
                                result['stage'] = "deploymentStatus";
...
function validateSession(orgId) {
    var org = orgsList.find((org) => org.orgId === orgId);
...
function cancelDeploy(orgId, deployJobId) {
    return new Promise((resolve, reject) => {
...
function quickDeploy(orgId, deployJobId) {
    return new Promise((resolve, reject) => {
...
function deployStatus(orgId, deployJobId) {
    return new Promise((resolve, reject) => {
...
function deploy(orgId, zipfile, checkOnly, testLevel, testClasses) {
    return new Promise((resolve, reject) => {
...
function deleteMD(orgId, packagexml) {
    var org = orgsList.find((org) => org.orgId === orgId);
...
function retrieveStatus(orgId, retrieveJobId) {
    return new Promise((resolve, reject) => {
...
function retrieve(orgId, packagexml) {
    return new Promise((resolve, reject) => {
...
function getTypesComponents(orgId, globalStorageUri, panel) {
    var org = orgsList.find((org) => org.orgId === orgId);
    return new Promise((resolve, reject) => {
                        ...
                        return sendSoapAPIRequest(orgId, '<urn:describeGlobal/>')
                            ...
                            if (objects.length > 0) {
                                ...
                                return Promise.all(chunks.map((chunk) => {
                                    ...
                                    fs.writeFile(fieldsPath, JSON.stringify(Array.from(sobjects), null, 2), 'utf8', (err) => { });
                                }).catch(error => {
                                    vscode.window.showErrorMessage(`Error ${error}`);
                                });
                            }
...
function sendSoapMDRequest(orgId, body) {
...
function sendSoapAPIRequest(orgId, body) {
    let org = orgsList.find((org) => org.orgId === orgId);
    const parser = new xml2js.Parser({ explicitArray: false, ignoreAttrs: true });
            ...
            if (error) {
                ...
                try {
                    ...
                    const newOrgs = orgs.filter((org) => {
                        ...
                        const isNew = !orgsList.find((o) => o.orgId === org.orgId);
                    ...
                    const displayPromises = newOrgs.map((org) => {
                        return new Promise((res) => {
                            exec(`sf org display --target-org ${org.username} --verbose --json`, (err, out) => {
                                if (err) {
